...
DEFINE_SPINLOCK(sem_lock); // Defining spinlock
struct Node
{
...
asmlinkage long sys_cs1550_down(struct cs1550_sem *sem){
	...
	if(sem->val < 0){
		...
		if(sem->s != NULL){
			...
			sem->s = n;
			//if it is empty, put it at the front
		}
		sem->e = n;
		//Set it at the end

		//End adding process to the queue
...
asmlinkage long sys_cs1550_up(struct cs1550_sem *sem){
	...
	if(sem->val <= 0){
		...
		{
			tTask = n->task;
			//checks to see if the list was empty

			if (n == sem->e)
			{
				sem->s = NULL;
				sem->e = NULL;
				//Checking to see if current node is at tne end of the list
			}
			else
				sem->s = n->nextNode;
		...
		kfree(n);
		//Free the memory taken up by the node's position in the list
